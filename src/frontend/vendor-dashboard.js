...
const hasOrders = !!ordersPanel;
const hasInventory = !!inventoryPanel;
const hasPos = !!posPanel;
const hasAnalytics = !!analyticsPanel;
const hasNotifications = !!notificationsPanel;
...
`<p>Low stock: ${item.name || item.productId}</p>`
...
`<p>Order update: ${order.id} ${order.status}</p>`
...
notificationsPanel.innerHTML =
'<h3>Notifications</h3><p>No notifications yet.</p>';
